...
# Name: DB reconnect wrapper (обёртка переподключения базы)
# ============================================================

def run_pg(operation, retries: int = 2, delay: float = 0.4):
    last_error = None
...
# ============================================================
# Name: PostgreSQL ping (проверка соединения PostgreSQL)
# ============================================================
...
def find_best_duplicate_pg(title_or_text: str, keywords: list[str]):
    ...
    query_norm = normalize_compare_text(query_raw)
    ...
    for rec in records:
        ...
        db_short_norm = normalize_compare_text(short_raw)
        db_full_norm = normalize_compare_text(full_raw[:2500])
        db_all_norm = normalize_compare_text(db_text_raw)
        ...
        if query_norm and db_all_norm:
            if query_norm[:500] in db_all_norm or db_all_norm[:500] in query_norm:
                containment_score = 1.0
            elif query_norm[:250] in db_all_norm or db_all_norm[:250] in query_norm:
                containment_score = 0.85

        core_overlap = 0.0
        if query_kw and db_kw:
            common = query_kw.intersection(db_kw)
            important_common = [
                w for w in common
                if len(w) >= 5
            ]
...
def create_idea_pg(data: dict):
    ...
    idea_fields = {
        ...
        'Confidentiality Level': data.get('confidentiality_level'),
        'Readiness Level': data.get('readiness_level'),

        'Source': data.get('source', 'Simple'),
        'Patentability': data.get('patentability'),
        'IsPatentable': data.get('is_patentable'),
        'Existing Analogues': data.get('existing_analogues'),

        'Notes': data.get('notes'),
        'NotesAI': data.get('notes_ai'),
        'TagsAI': data.get('ai_tags'),
        'AIRequest': data.get('ai_request', ''),

        'Similarity Score': data.get('similarity_score', 0),
        'DuplicateOf': data.get('related_to_id'),
        'RelationType': data.get(
            'relation_type',
            'possible_duplicate' if data.get('related_to_id') else None
        ),

        'DetectedByAI': bool(data.get('detected_by_ai', False)),
        'DiscoveredByAI': bool(data.get('discovered_by_ai', False)),

        'AIReviewStatus': data.get('ai_review_status', 'Done'),
        ...
        'checked_by_role': data.get('checked_by_role'),
        'checked_by_name': data.get('checked_by_name'),
        'checked_by_email': data.get('checked_by_email'),
        'checked_at': data.get('checked_at'),

        'requires_moderation': bool(data.get('requires_moderation', False)),
        'visibility_status': data.get('visibility_status', 'internal'),

        'collector_session_id': data.get('collector_session_id'),
        'collector_source_filename': data.get('collector_source_filename'),
        'collector_source_url': data.get('collector_source_url'),
        'collector_source_type': data.get('collector_source_type'),
        'collector_raw_agent_result': json.dumps(
            data.get('collector_raw_agent_result'),
            ensure_ascii=False
        ) if data.get('collector_raw_agent_result') is not None else None,
